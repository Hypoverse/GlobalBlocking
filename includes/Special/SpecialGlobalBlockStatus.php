...
class SpecialGlobalBlockStatus extends FormSpecialPage {
	...
	private function loadParameters( ?string $par ) {
		...
		if ( $ip !== '' || $request->wasPosted() ) {
			if ( IPUtils::isIPAddress( $ip ) ) {
				$ip = IPUtils::sanitizeIP( $ip );
			} else {
				$ip = $this->userNameUtils->getCanonical( $ip );
				$centralId = ( $ip !== false ) ? $this->centralIdLookup->centralIdFromName( $ip ) : 0;
			}
		}

		// $ip could be empty string or it could be boolean false (from UserNameUtils::getCanonical);
		// normalize both of these cases to null for ease of later checks.
		$this->mAddress = $ip ?: null;
		$this->mCentralId = $centralId ?: null;

		$this->mWhitelistStatus = $request->getCheck( 'wpWhitelistStatus' );

		if ( $this->mAddress ) {
			$id = GlobalBlocking::getGlobalBlockId( $this->mAddress, $centralId );
			$allowlistInfo = GlobalBlocking::getLocalWhitelistInfo( $id, $this->mAddress, $centralId );
			$this->mCurrentStatus = ( $allowlistInfo !== false );
	...
	public function onSubmit( array $data ) {
		$ip = $this->mAddress;
		$centralId = $this->mCentralId;

		$id = GlobalBlocking::getGlobalBlockId( $ip, $centralId );
		// Is it blocked?
		if ( !$id ) {
		...
		if ( $this->mWhitelistStatus == true ) {
			...
			if ( $this->config->get( 'GlobalBlockingApplyUsernameBlocks' ) ) {
				// schema change to introduce this field may not have run yet;
				// gate it behind the $wgGlobalBlockingApplyUsernameBlocks flag
				$row['gbw_central_id'] = $centralId;
			}

			if ( GlobalBlocking::getLocalWhitelistInfoByTarget( $this->mAddress ) !== false ) {
