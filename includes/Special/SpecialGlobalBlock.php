...
class SpecialGlobalBlock extends FormSpecialPage {
	...
	/** @var int|null One of the AbstractBlock::TYPE_ constants */
	protected $type;
	...
	public function execute( $par ) {
		...
		$out->addModules( 'ext.GlobalBlocking.special.globalblock' );
	}
	...
	protected function setParameter( $par ) {
		...
		} elseif ( IPUtils::isValid( $address ) ) {
			$this->address = IPUtils::sanitizeIP( $address );
		} else {
			// If the username is invalid, getCanonical returns false; however we want
			// to coerce that to null so that parseBlockTarget short-circuits properly
			$this->address = $this->userNameUtils->getCanonical( $address ) ?: null;
		}

		[ $target, $this->type ] = $this->blockUtils->parseBlockTarget( $this->address );
	...
	protected function loadExistingBlock() {
		...
		if ( $this->address ) {
			...
			if ( IPUtils::isIPAddress( $this->address ) ) {
				$conds['gb_address'] = $this->address;
			} else {
				$userId = $this->centralIdLookup->centralIdFromName( $this->address );
				if ( $userId === 0 ) {
					// can't be globally blocked if they don't have a central user account
					return $blockOptions;
				}

				$conds['gb_user_central_id'] = $userId;
			}
	...
	public function onSubmit( array $data ) {
		...
		$enableSoftBlock = in_array( $this->type, [ AbstractBlock::TYPE_IP, AbstractBlock::TYPE_RANGE ] );

		if ( $enableSoftBlock && $data['AnonOnly'] ) {
			$options[] = 'anon-only';
		}
		...
		$errors = GlobalBlocking::block(
			// $this->address is sanitized; $data['Address'] isn't
			$this->address,
			$data['Reason'][0],
			$data['Expiry'],
			$performer,
		...
		if ( $performer->isAllowed( 'block' ) && $data['AlsoLocal'] ) {
			$status = $this->blockUserFactory->newBlockUser(
				...
				[
					...
					'isHardBlock' => !( $enableSoftBlock && $data['AlsoLocalSoft'] ),
					'isAutoblocking' => true,
				]
